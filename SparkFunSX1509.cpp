...
unsigned char SX1509::begin(unsigned char address, unsigned char resetPin)
{
	begin(nullptr, address, resetPin);
}

unsigned char SX1509::begin(const char* device, unsigned char address, unsigned char resetPin)
{
...
unsigned char SX1509::init(const char* device)
{
	// Begin I2C
	//Wire.begin();
	if(device == nullptr)
	...
	// Then read a unsigned char that should be 0x00
...
void SX1509::reset(bool hardware)
{
	...
	if (hardware)
	{
		...
		unsigned char regMisc = readByte(REG_MISC);
		if (regMisc & (1<<2))
		{
			regMisc &= ~(1<<2);
...
void SX1509::pinDir(unsigned char pin, unsigned char inOut)
{
...
void SX1509::pinMode(unsigned char pin, unsigned char inOut)
{
	pinDir(pin, inOut);
}

void SX1509::writePin(unsigned char pin, unsigned char highLow)
{
	unsigned int tempRegDir = readWord(REG_DIR_B);
...
void SX1509::digitalWrite(unsigned char pin, unsigned char highLow)
{
	writePin(pin, highLow);
}

unsigned char SX1509::readPin(unsigned char pin)
{
	unsigned int tempRegDir = readWord(REG_DIR_B);
...
unsigned char SX1509::digitalRead(unsigned char pin)
{
	return readPin(pin);
}

void SX1509::ledDriverInit(unsigned char pin, unsigned char freq /*= 1*/, bool log /*= false*/)
{
	...
	if (_clkX == 0) // Make clckX non-zero
	{
		...
		unsigned char freq = (1 & 0x07) << 4;	// freq should only be 3 bits from 6:4
		tempByte |= freq;
	}
...
void SX1509::pwm(unsigned char pin, unsigned char iOn)
{
...
void SX1509::analogWrite(unsigned char pin, unsigned char iOn)
{
	pwm(pin, iOn);
}

void SX1509::blink(unsigned char pin, unsigned long tOn, unsigned long tOff, unsigned char onIntensity, unsigned char offIntensity)
{
	unsigned char onReg = calculateLEDTRegister(tOn);
	unsigned char offReg = calculateLEDTRegister(tOff);
	
	setupBlink(pin, onReg, offReg, onIntensity, offIntensity, 0, 0);
}

void SX1509::breathe(unsigned char pin, unsigned long tOn, unsigned long tOff, unsigned long rise, unsigned long fall, unsigned char onInt, unsigned char offInt, bool log)
{
	...
	unsigned char riseTime = calculateSlopeRegister(rise, onInt, offInt);
	unsigned char fallTime = calculateSlopeRegister(fall, onInt, offInt);
	
	setupBlink(pin, onReg, offReg, onInt, offInt, riseTime, fallTime, log);
}

void SX1509::setupBlink(unsigned char pin, unsigned char tOn, unsigned char tOff, unsigned char onIntensity, unsigned char offIntensity, unsigned char tRise, unsigned char tFall, bool log)
{
...
void SX1509::keypad(unsigned char rows, unsigned char columns, unsigned int sleepTime, unsigned char scanTime, unsigned char debounceTime)
{
	unsigned int tempWord;
	unsigned char tempByte;
...
unsigned char SX1509::getRow(unsigned int keyData)
{
	unsigned char rowData = (unsigned char)(keyData & 0x00FF);
...
unsigned char SX1509::getCol(unsigned int keyData)
{
	unsigned char colData = (unsigned char)((keyData & 0xFF00) >> 8);
...
void SX1509::debounceConfig(unsigned char configValue)
{
	// First make sure clock is configured
	unsigned char tempByte = readByte(REG_MISC);
	if ((tempByte & 0x70) == 0)
	{
...
void SX1509::debounceTime(unsigned char time)
{
	if (_clkX == 0) // If clock hasn't been set up.
		clock(INTERNAL_CLOCK_2MHZ, 1); // Set clock to 2MHz.
	
	// Debounce time-to-unsigned char map: (assuming fOsc = 2MHz)
	// 0: 0.5ms		1: 1ms
	// 2: 2ms		3: 4ms
	// 4: 8ms		5: 16ms
	// 6: 32ms		7: 64ms
...
void SX1509::debounceEnable(unsigned char pin)
{
...
void SX1509::debouncePin(unsigned char pin)
{
	debounceEnable(pin);
}

void SX1509::debounceKeypad(unsigned char time, unsigned char numRows, unsigned char numCols)
{
...
void SX1509::enableInterrupt(unsigned char pin, unsigned char riseFall)
{
	...
	// 10: Falling
	// 11: Both
	unsigned char pinMask = (pin & 0x07) * 2;
	unsigned char senseRegister;
...
void SX1509::clock(unsigned char oscSource, unsigned char oscDivider, unsigned char oscPinFunction, unsigned char oscFreqOut)
{
	configClock(oscSource, oscPinFunction, oscFreqOut, oscDivider);
}

void SX1509::configClock(unsigned char oscSource /*= 2*/, unsigned char oscPinFunction /*= 0*/, unsigned char oscFreqOut /*= 0*/, unsigned char oscDivider /*= 1*/)
{
	...
	unsigned char regClock = oscSource | oscPinFunction | oscFreqOut;
...
unsigned char SX1509::calculateLEDTRegister(int ms)
{
	int regOn1, regOn2;
...
unsigned char SX1509::calculateSlopeRegister(int ms, unsigned char onIntensity, unsigned char offIntensity)
{
...
// readByte(unsigned char registerAddress)
//	This function reads a single unsigned char located at the registerAddress register.
//	- deviceAddress should already be set by the constructor.
//	- Return value is the unsigned char read from registerAddress
//		- Currently returns 0 if communication has timed out
unsigned char SX1509::readByte(unsigned char registerAddress)
{
	...
	unsigned char readValue = static_cast<unsigned char>(wiringPiI2CRead(wiringPiFd));
	//Wire.requestFrom(deviceAddress, (unsigned char) 1);
...
// readWord(unsigned char registerAddress)
//	This function will read a two-unsigned char word beginning at registerAddress
...
unsigned int SX1509::readWord(unsigned char registerAddress)
{
...
//	Wire.write(registerAddress);
//	Wire.endTransmission();
//	Wire.requestFrom(deviceAddress, (unsigned char) 2);
//
...
// writeByte(unsigned char registerAddress, unsigned char writeValue)
//	This function writes a single unsigned char to a single register on the SX509.
//	- writeValue is written to registerAddress
//	- deviceAddres should already be set from the constructor
//	- No return value.
void SX1509::writeByte(unsigned char registerAddress, unsigned char writeValue)
{
//	Wire.beginTransmission(deviceAddress);
//	Wire.write(registerAddress);
...
// writeWord(unsigned char registerAddress, ungisnged int writeValue)
//	This function writes a two-unsigned char word to registerAddress and registerAddress + 1
//	- the upper unsigned char of writeValue is written to registerAddress
//		- the lower unsigned char of writeValue is written to registerAddress + 1
//	- No return value.
void SX1509::writeWord(unsigned char registerAddress, unsigned int writeValue)
{
